...
Field::Field(string name_of_input_file, int dimension) {
    this->_dimension = dimension;
    _field = new ArraySequence<char>;
    for (int i = 0; i < dimension * dimension; ++i) {_field->Append('0');}
...
char Field::CheckFinishStateBigField() {
    ...
    for (int i = 0; i < _dimension; ++i) {   // horizontal
        for (int k = 0; k < _dimension - dimension + 1; ++k) {
            ...
            for (int j = 0; j < dimension - 1; ++j) {
                ...
                if ((*_field)[j + i * _dimension + k] != (*_field)[j + 1 + i * _dimension + k]) {
                    checker = false;
                    break;
                }
            }
            if (checker && (*_field)[i * _dimension + k] != '0') {
                //return (*_field)[i * _dimension];
                return (*_field)[i * _dimension + k];
            }
        }
    }
    ...
    for (int i = 0; i < _dimension; ++i) {   // vertical
        //for (int k = 0; k < 1; ++k) {
        for (int k = 0; k < _dimension - dimension + 1; ++k) {
        //for (int k = 0; k < _dimension - dimension; ++k) {
            checker = true;
            //for (int j = 0; j < _dimension - 1; ++j) {
            for (int j = 0; j < dimension - 1; ++j) {
            ...
            if (checker && (*_field)[i + k * _dimension] != '0') {
                return (*_field)[i + k * _dimension];
            }
        }
    }
    ...
    for (int y = 0; y < _dimension - dimension + 1; ++y) {
        for (int x = 0; x < _dimension - dimension + 1; ++x) {
            ...
            for (int i = 0; i < dimension - 1; ++i) {
                ...
                if ((*_field)[(i + x) + (i + y) * _dimension] != (*_field)[(i + 1 + x) + (i + 1 + y) * _dimension]) {
                    checker = false;
                    break;
                }
            }
            if (checker && (*_field)[x + y * dimension] != '0') {
                return (*_field)[x + y * dimension];
            }
        }
    }
    ...
    for (int y = 0; y < _dimension - dimension + 1; ++y) {
        for (int x = 0; x < _dimension - dimension + 1; ++x) {
            ...
            for (int i = 0; i < dimension - 1; ++i) {
                ...
                if ((*_field)[(_dimension - i - 1 + x) + (i + y) * _dimension] != (*_field)[(_dimension - i - 2 + x) + (i + 1 + y) * _dimension]) {
                    checker = false;
                    break;
                }
            }
            //if (checker && (*_field)[dimension - 1 + y * _dimension] != '0') {
            if (checker && (*_field)[_dimension - 1 + y * _dimension] != '0') {
                //return (*_field)[dimension - 1 + y * _dimension];
                return (*_field)[_dimension - 1 + y * _dimension];
            }
        }
    }
    return '0';
