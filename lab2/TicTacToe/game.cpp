...
void Game::Init(string name_of_state_file, string name_of_mode_file) {
    InitMode(name_of_mode_file);
    LoadInitStateGameByFile(name_of_state_file);
}

void Game::MakeTree(TreeItem<GameState> *root, char mode) {
    bool checker = true;
    if (!root) {
        root = _tree->GetRoot();
    }
...
Point Game::FindBestXStep() {
    int index = 0;
    for (int i = 0; i < _win_chances->GetLength(); ++i) {
        index = ((*_win_chances)[index] >= (*_win_chances)[i]) ? index:i;
    }
    Point p;
    p.y = index / _dimension;
    p.x = index - p.y * _dimension;
    return p;
}

int Game::ChancesBestXStep() {
    Point p = FindBestXStep();
    return (*_win_chances)[p.x + p.y * _dimension];
}

void Game::SaveChancesInFile(string name_of_file) {
...
void Game::SaveStepInFile(string name_of_file) {
    if (_mode == 1)
